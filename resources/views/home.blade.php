{{--
    ...
    Karya ini dilindungi sepenuhnya oleh hak cipta. Penggunaan tanpa izin
    tertulis dari pemegang hak cipta DILARANG KERAS.

    Lihat file WATERMARK.md untuk informasi lengkap tentang perlindungan harta cipta.
    ============================================================================
--}}

@extends('layouts.app')

@section('title', 'Toolbox - Semua Tools Online dalam Satu Tempat')
...
<div x-data="{ showModal: false }" x-cloak>
    ...
    <div x-show="showModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4"
        ...
        <div class="bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl rounded-3xl shadow-xl max-w-md w-full p-8"
            ...
            <div class="space-y-3">
                <a href="{{ route('login') }}" class="block w-full bg-gradient-to-r from-primary-600 to-purple-600 text-white text-center py-4 rounded-xl font-medium hover:shadow-xl hover:scale-105 transition-all duration-300">
                    Masuk
                </a>
                <a href="{{ route('register') }}" class="block w-full border-2 border-primary-600 dark:border-primary-500 text-primary-600 dark:text-primary-400 text-center py-4 rounded-xl font-medium hover:bg-primary-50 dark:hover:bg-primary-950 transition-all duration-300">
                    Daftar Akun Baru
                </a>
            ...
            <a href="#" @click.prevent="showModal = true" class="group relative bg-gradient-to-br from-white/80 to-white/60 dark:from-gray-800/80 dark:to-gray-800/60 backdrop-blur-xl rounded-3xl p-6 hover:shadow-2xl hover:shadow-blue-500/10 dark:hover:shadow-blue-500/5 hover:scale-105 hover:-translate-y-1 transition-all duration-500 border border-gray-200/50 dark:border-gray-700/50 hover:border-blue-500/50 dark:hover:border-blue-500/30 overflow-hidden">
                ...
                <div class="relative z-10">
                    <div class="flex items-start justify-between mb-4">
                        <div class="text-5xl group-hover:scale-125 group-hover:rotate-6 transition-all duration-500">{{ $tool->category->icon }}</div>
                        @if($tool->is_new)
                    ...
                    <p class="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">{{ $tool->description }}</p>
                </div>
            </a>
            @endforeach
        </div>
        ...
        <div class="mb-16">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4 group">
                <div class="flex items-center gap-4">
                    <div class="text-5xl md:text-6xl group-hover:scale-110 transition-transform duration-500">{{ $category->icon }}</div>
                    <div>
                        <h3 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white tracking-tight">{{ $category->name }}</h3>
                        <p class="text-gray-600 dark:text-gray-400 text-sm mt-1 font-light">{{ $category->description }}</p>
                    </div>
                </div>
                <a href="{{ route('categories.show', $category->slug) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-medium flex items-center gap-2 group text-sm">
                    Lihat semua
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                @foreach($category->activeTools as $tool)
                    ...
                    <div class="relative z-10">
                        <div class="flex items-start justify-between">
                            <div class="flex-1">
                                ...
                                <p class="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">{{ Str::limit($tool->description, 60) }}</p>
                            </div>
                            @if($tool->is_new)
        ...
        <div class="card-hover group relative">
            ...
            <a href="{{ route('tools.show', $tool->slug) }}" class="block">
                <div class="flex items-start justify-between mb-4 pr-8">
                    <div class="text-4xl transform group-hover:scale-110 transition-transform">{{ $tool->category->icon }}</div>
                    @if($tool->is_new)
...
<section>
    ...
    <div class="mb-12">
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-4">
                <div class="text-4xl md:text-5xl">{{ $category->icon }}</div>
                <div>
                    <h3 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">{{ $category->name }}</h3>
                    <p class="text-gray-600 dark:text-gray-400 text-sm mt-1">{{ $category->description }}</p>
                </div>
            </div>
            <a href="{{ route('categories.show', $category->slug) }}" class="text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 font-medium flex items-center gap-2 group">
                Lihat semua
                ...
                <a href="{{ route('tools.show', $tool->slug) }}" class="block">
                    <div class="flex items-start justify-between pr-8">
                        <div class="flex-1">
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400">{{ Str::limit($tool->description, 60) }}</p>
                        </div>
                        @if($tool->is_new)
